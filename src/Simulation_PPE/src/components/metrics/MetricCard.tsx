  ...
  return (
    <div className="bg-slate-50 p-3 rounded-md border border-gray-100">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div>
          <div className="text-gray-500">Théorie des Jeux</div>
          <div className="font-bold text-emerald-600">{formatNumber(gameTheory)} {unit}</div>
        </div>
        <div>
          <div className="text-gray-500">Temps Fixe</div>
      ...
      <div className={`mt-2 text-xs font-medium ${isImprovement ? 'text-green-600' : 'text-amber-600'}`}>
        {isImprovement ? (
          <span>Amélioration de {absImprovement.toFixed(1)}%</span>
        ) : (
          <span>Diminution de {absImprovement.toFixed(1)}%</span>
        )}
      </div>
    </div>
  );
